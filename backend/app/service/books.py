...
class BookDto:
    def __init__(self, id: str, title: str,
                 author_name: str,
                 publish_year: int, tags: List[str],
                 cover_link: str):
        self.id = id
        self.title = title
    ...
    def from_list_dict(cls, data: dict):
        ...
        if cover is not None:
            cover_link = 'https://covers.openlibrary.org/b/id/'\
                  + str(cover) + '-M.jpg'
        try:
            return cls(
                id=data.get("editions").get("docs")[0]
                .get("key").split('/')[-1],
                title=data.get('title'),
                author_name=", ".join(data.get('author_name', ['']))
                            if isinstance(data.get('author_name'), list)
                            and data.get('author_name') else '',
...
class Books:
    ...
    async def get_book_by_id(id: str):
        ...
        async with httpx.AsyncClient() as client:
            ...
            for author in authors:
                response_author = await client.get(BASE_URL+author.get("key")
                                                   + ".json")
            ...
            if works is not None and len(works) != 0:
                response_tags = await client.get(BASE_URL+works[0].get("key")
                                                 + ".json")
                response_tags.raise_for_status()
                tags_data = response_tags.json()
                data["subjects"] = tags_data.get("subjects")\
                    if tags_data.get("subjects") else []
    ...
    @staticmethod
    async def search_books(name: str, author: str, tags: [str],
                           page: int, size: int):
        ...
        params['page'] = page
        params['limit'] = size
        params['fields'] = "key,title,author_name,first_publish_year"\
            + ",editions,cover_i, subject"
        async with httpx.AsyncClient() as client:
            response = await client.get(url, params=params)
            response.raise_for_status()
            data = response.json()
            books = [BookDto.from_list_dict(book) for book in
                     data.get('docs', [])]
